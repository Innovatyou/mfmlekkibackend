...
function delete_church(e){
  var id = e.target.getAttribute('data-id');
  var action = e.target.getAttribute('data-action');
  var msg = action == 1? "If you click on the red button below, all church data and associated files will be cleared from your system within the next 24hours. This action cannot be reversed!!!":'Click the green button below to cancel this church data from being deleted.';
  var url = '';
   swal({
     title: action == 1?'Delete church?':"Cancel delete?",
     text: msg,
     type: 'warning',
     confirmButtonColor: action == 1?"#DD6B55":"#088F8F",
     showCancelButton: true,
     confirmButtonText: 'Yes, Sure'
   },function () {
       document.location.href = baseURL+'/schedulechurchdeleteaction/'+id+'/'+action;
   });
}
...
$('#transactions_table').DataTable({
  "bProcessing": true,
   "serverSide": true,
    "pageLength" : 10,
    "ajax": {
        url : baseURL+"/transactionslisting",
        type : 'POST'
    },
    dom: 'frtip'
});
...
function uploadNewVideo(event){
  ...
  if(title == ""){
   ...
   if(media_type == "video_link" && !isValidVideoURL(media_link)){
