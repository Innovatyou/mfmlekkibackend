...
class Hymns extends BaseController
{
  ...
  {
    //$data['userRecords'] = $hymnsmodel->usersListing();
    return $this->view("hymns/listing", $this->viewdata);
  }
  ...
  {
    return $this->view("hymns/new", $this->viewdata);
  }
  ...
  public function editHymn($id = 0)
  {
    ...
    return $this->view("hymns/edit", $this->viewdata);
  }
  ...
  function saveNewHymn()
  {
    ...
    $hymnsmodel->addNewHymn($info);
    if ($hymnsmodel->status == "ok") {
      $this->session->setFlashdata('success', $hymnsmodel->message);
    } else {
      $this->session->setFlashdata('error', $hymnsmodel->message);
    }
