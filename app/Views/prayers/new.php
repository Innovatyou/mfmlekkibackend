  ...
  <div class="xs-pd-20-10 pd-ltr-20">
    <div class="page-header">
      <div>
        ...
        <nav class="lt-bc"><a href="<?= base_url() ?>">Dashboard</a><span>/</span><a href="<?= base_url('prayersListing') ?>"><?= $locale['prayer_requests'] ?></a><span>/</span><span><?= $locale['new_request'] ?></span></nav>
      </div>
    </div>
    <?php if(session()->getFlashdata('success')):?><div class="lt-alert lt-success"><i class="dw dw-check-circle-2"></i><?=esc(session()->getFlashdata('success'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <?php if(session()->getFlashdata('error')):?><div class="lt-alert lt-danger"><i class="dw dw-close-circle-1"></i><?=esc(session()->getFlashdata('error'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <form method="POST" action="<?= base_url('saveNewPrayer') ?>">
      <?= csrf_field() ?>
      <div class="row">
        <div class="col-lg-8">
          <div class="nf-card">
            <div class="nf-card-head"><h3 class="nf-card-title">Prayer Request Details</h3></div>
            <div class="nf-card-body">
              <div class="nf-row" style="margin-bottom:16px;">
                <div class="nf-col-half">
                  <label class="nf-label"><?= $locale['requester'] ?></label>
                  <input type="text" name="requester" class="nf-input" placeholder="<?= $locale['requester'] ?>">
                </div>
                <div class="nf-col-half">
                  <label class="nf-label"><?= $locale['prayer_visibility'] ?></label>
                  <select name="public" class="nf-input nf-select">
                    <option value="0"><?= $locale['public'] ?></option>
                    <option value="1" selected><?= $locale['private'] ?></option>
                  </select>
                </div>
              </div>
              <div style="margin-bottom:16px;">
                <label class="nf-label"><?= $locale['request_title'] ?></label>
                <input type="text" name="title" class="nf-input" placeholder="<?= $locale['request_title'] ?>" required>
              </div>
              <div>
                <label class="nf-label"><?= $locale['request_content'] ?></label>
                <textarea class="editor1" name="content"><?= $locale['add_request_content'] ?></textarea>
              </div>
            </div>
          </div>
        ...
        <div class="col-lg-4">
          <div class="nf-card" style="background:linear-gradient(135deg,#fdf4ff,#faf5ff);">
            <div class="nf-card-body" style="padding:24px;">
              <div style="display:flex;align-items:flex-start;gap:14px;">
                <div style="width:40px;height:40px;border-radius:12px;background:#9333ea;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><i class="dw dw-prayer" style="color:#fff;font-size:1rem;"></i></div>
                <div>
                  <p style="font-size:.85rem;font-weight:700;color:#6b21a8;margin:0 0 6px;">Visibility</p>
                  <p style="font-size:.78rem;color:#9333ea;margin:0 0 10px;line-height:1.5;"><strong>Public</strong> — visible to all app members in the community prayer wall.</p>
                  <p style="font-size:.78rem;color:#9333ea;margin:0;line-height:1.5;"><strong>Private</strong> — only visible to church admins and prayer team.</p>
                </div>
              </div>
            </div>
          </div>
