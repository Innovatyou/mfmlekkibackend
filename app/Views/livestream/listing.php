    ...
    <div class="page-header">
      <div>
        ...
        <nav class="ml-breadcrumb">
          ...
          <span>/</span>
          <span><?= $locale['livestream_channels'] ?></span>
        </nav>
      </div>
      <a href="<?= base_url('newLivestream') ?>" class="btn btn-primary" style="border-radius:8px;font-weight:600;padding:9px 20px;font-size:.875rem;">
        <i class="dw dw-add" style="margin-right:6px;"></i><?= $locale['new_livestream'] ?>
      </a>
    </div>
    ...
    <div class="card-box" style="padding:0;overflow:hidden;">
      ...
      <div style="padding:16px 22px 22px;overflow-x:auto;">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                <?php if ($record->id == 1): ?>
                  <span class="ml-badge ml-badge-primary" style="margin-left:6px;">Primary</span>
                <?php endif; ?>
              </td>
              <td>
              ...
              <td class="text-center">
                <?php if ($record->id == 1): ?>
                  <span title="Primary channel cannot be deleted" style="color:var(--t3);font-size:.85rem;">
                    <i class="dw dw-padlock-1"></i>
                  </span>
                <?php else: ?>
                  <div style="display:flex;gap:5px;justify-content:center;">
                    <a href="<?= base_url('editLivestream/' . $record->id) ?>" class="ml-action-btn ml-action-edit" title="Edit">
                      <i class="dw dw-edit-2"></i>
                    </a>
                    <a href="javascript:void(0)" class="ml-action-btn ml-action-delete" title="Delete"
                       onclick="confirmDeleteLivestream(<?= $record->id ?>)">
        ...
        <?php if (empty($livestreams)): ?>
          <div style="padding:40px;text-align:center;color:var(--t3);font-size:.875rem;">No livestream channels found</div>
        <?php endif; ?>
      </div>
    </div>
...
<style>
  ...
  #livestreams_table tbody td {
    padding:12px 14px;border-color:var(--border) !important;
    font-size:.875rem;vertical-align:middle;
  }
  #livestreams_table tbody tr:hover td { background:#f8fafc; }
  #livestreams_table tbody tr:last-child td { border-bottom:none !important; }
  ...
  .ml-badge { display:inline-flex;align-items:center;padding:2px 8px;border-radius:20px;font-size:.7rem;font-weight:700; }
  .ml-badge-primary { background:#eef2ff;color:#6366f1; }
...
<script>
function initLivestreamsTable() {
  if (typeof $ === 'undefined' || typeof $.fn === 'undefined' || typeof $.fn.DataTable === 'undefined') {
    return;
  }

  if ($.fn.DataTable.isDataTable('#livestreams_table')) {
    $('#livestreams_table').DataTable().destroy();
  }
...
window.addEventListener('load', initLivestreamsTable);

function confirmDeleteLivestream(id) {
  swal({
