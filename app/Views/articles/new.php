  ...
  <div class="xs-pd-20-10 pd-ltr-20">
    <div class="page-header">
      <div>
        ...
        <nav class="lt-bc"><a href="<?= base_url() ?>">Dashboard</a><span>/</span><a href="<?= base_url('articlesListing') ?>"><?= $locale['articles'] ?></a><span>/</span><span><?= $locale['add_new'] ?></span></nav>
      </div>
    </div>
    <?php if(session()->getFlashdata('success')):?><div class="lt-alert lt-success"><i class="dw dw-check-circle-2"></i><?=esc(session()->getFlashdata('success'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <?php if(session()->getFlashdata('error')):?><div class="lt-alert lt-danger"><i class="dw dw-close-circle-1"></i><?=esc(session()->getFlashdata('error'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <form method="POST" action="<?= base_url('saveNewArticle') ?>" enctype="multipart/form-data">
      <?= csrf_field() ?>
      <div class="row">
        <div class="col-lg-8">
          <div class="nf-card">
            ...
            <div class="nf-card-body">
              <div class="nf-row" style="margin-bottom:16px;">
                ...
                <div class="nf-col-half">
                  ...
                  <input type="text" name="author" class="nf-input" placeholder="<?= $locale['article_writer'] ?>" required>
                </div>
              </div>
              <div>
                <label class="nf-label"><?= $locale['article_title'] ?></label>
                <input type="text" name="title" class="nf-input" placeholder="<?= $locale['article_title'] ?>" required>
              </div>
            </div>
          </div>
          ...
          <div class="nf-card" style="margin-top:20px;">
            ...
            <div class="nf-card-body">
              <textarea class="editor" name="content"><?= $locale['article_content_here'] ?></textarea>
            </div>
          </div>
        ...
        <div class="col-lg-4">
          <div class="nf-card">
            <div class="nf-card-head"><h3 class="nf-card-title"><?= $locale['article_cover'] ?></h3><p class="nf-card-sub">Article featured image</p></div>
            <div class="nf-card-body" style="padding-top:8px;">
              <div class="nf-upload-zone" id="art-cover-zone" onclick="document.getElementById('art-cover-input').click()">
                ...
                <p class="nf-upload-hint">PNG, JPG — 16:9 recommended</p>
              </div>
              <input type="file" id="art-cover-input" name="thumbnail" accept="image/png,image/jpeg" style="display:none;" onchange="nfPreviewArt(this)">
            </div>
          </div>
        </div>
      </div>
