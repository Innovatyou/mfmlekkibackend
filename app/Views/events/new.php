  ...
  <div class="xs-pd-20-10 pd-ltr-20">
    <div class="page-header">
      <div>
        ...
        <nav class="lt-bc"><a href="<?= base_url() ?>">Dashboard</a><span>/</span><a href="<?= base_url('eventsListing') ?>"><?= $locale['events'] ?></a><span>/</span><span><?= $locale['new_event'] ?></span></nav>
      </div>
    </div>
    <?php if(session()->getFlashdata('success')):?><div class="lt-alert lt-success"><i class="dw dw-check-circle-2"></i><?=esc(session()->getFlashdata('success'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <?php if(session()->getFlashdata('error')):?><div class="lt-alert lt-danger"><i class="dw dw-close-circle-1"></i><?=esc(session()->getFlashdata('error'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <form method="POST" action="<?= base_url('savenewevent') ?>" enctype="multipart/form-data">
      <?= csrf_field() ?>
      <div class="row">
        <div class="col-lg-8">
          <div class="nf-card">
            ...
            <div class="nf-card-body">
              <div style="margin-bottom:16px;">
                ...
                <input type="text" name="title" class="nf-input" placeholder="<?= $locale['event_title'] ?>" required>
              </div>
              <div class="nf-row">
          ...
          <div class="nf-card" style="margin-top:20px;">
            <div class="nf-card-head"><h3 class="nf-card-title">Event Details</h3><p class="nf-card-sub">Full description and program details</p></div>
            <div class="nf-card-body">
              <textarea class="editor" name="details"><?= $locale['add_event_detail'] ?></textarea>
            </div>
          </div>

          <div style="margin-top:24px;display:flex;gap:12px;">
            <button type="submit" class="btn btn-primary nf-submit"><?= $locale['save_new'] ?></button>
            <a href="<?= base_url('eventsListing') ?>" class="btn btn-light nf-cancel">Cancel</a>
          </div>
        </div>

        <div class="col-lg-4">
          <div class="nf-card">
            <div class="nf-card-head"><h3 class="nf-card-title">Event Poster</h3><p class="nf-card-sub">Upload an event banner or poster</p></div>
            <div class="nf-card-body" style="padding-top:8px;">
              <div class="nf-upload-zone" id="evt-poster-zone" onclick="document.getElementById('evt-poster-input').click()">
                <div class="nf-upload-icon"><i class="dw dw-image"></i></div>
                <p class="nf-upload-text">Click to upload poster</p>
                <p class="nf-upload-hint">PNG, JPG — landscape recommended</p>
              </div>
              <input type="file" id="evt-poster-input" name="thumbnail" accept="image/png,image/jpeg" required style="display:none;" onchange="nfPreviewEvt(this)">
            </div>
          </div>
        </div>
      </div>
...
.nf-upload-zone{border:2px dashed var(--border);border-radius:10px;padding:28px 16px;text-align:center;cursor:pointer;transition:border-color .15s,background .15s;}
...
<script>
function nfPreviewEvt(input) {
  var zone = document.getElementById('evt-poster-zone');
  if (input.files && input.files[0]) {
    var reader = new FileReader();
    reader.onload = function(e) {
      zone.innerHTML = '<img src="'+e.target.result+'" style="max-height:140px;max-width:100%;border-radius:8px;object-fit:cover;" alt="Poster">';
    };
