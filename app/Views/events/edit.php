  ...
  <div class="xs-pd-20-10 pd-ltr-20">
    <div class="page-header">
      <div>
        ...
        <nav class="lt-bc"><a href="<?= base_url() ?>">Dashboard</a><span>/</span><a href="<?= base_url('eventsListing') ?>"><?= $locale['events'] ?></a><span>/</span><span><?= $locale['edit_details'] ?></span></nav>
      </div>
    </div>
    <?php if(session()->getFlashdata('success')):?><div class="lt-alert lt-success"><i class="dw dw-check-circle-2"></i><?=esc(session()->getFlashdata('success'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <?php if(session()->getFlashdata('error')):?><div class="lt-alert lt-danger"><i class="dw dw-close-circle-1"></i><?=esc(session()->getFlashdata('error'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <form method="POST" action="<?= base_url('editEventData') ?>" enctype="multipart/form-data">
      <?= csrf_field() ?>
      ...
      <div class="row">
        ...
        <div class="col-lg-4">
          <div class="nf-card">
            <div class="nf-card-head"><h3 class="nf-card-title">Event Poster</h3><p class="nf-card-sub">Click to change poster</p></div>
            <div class="nf-card-body" style="padding-top:8px;">
              ...
              <input type="file" id="evt-poster-input" name="thumbnail" accept="image/png,image/jpeg" style="display:none;" onchange="nfPreview(this,'evt-poster-zone')">
            </div>
          </div>
        </div>
      </div>
