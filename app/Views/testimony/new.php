  ...
  <div class="xs-pd-20-10 pd-ltr-20">
    <div class="page-header">
      <div>
        ...
        <nav class="lt-bc"><a href="<?= base_url() ?>">Dashboard</a><span>/</span><a href="<?= base_url('testimonyListing') ?>"><?= $locale['testimonies'] ?></a><span>/</span><span><?= $locale['new_testimony'] ?></span></nav>
      </div>
    </div>
    <?php if(session()->getFlashdata('success')):?><div class="lt-alert lt-success"><i class="dw dw-check-circle-2"></i><?=esc(session()->getFlashdata('success'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <?php if(session()->getFlashdata('error')):?><div class="lt-alert lt-danger"><i class="dw dw-close-circle-1"></i><?=esc(session()->getFlashdata('error'))?><button class="lt-x" onclick="this.parentElement.remove()">&times;</button></div><?php endif;?>
    <form method="POST" action="<?= base_url('savenewtestimony') ?>" enctype="multipart/form-data">
      <?= csrf_field() ?>
      <div class="row">
        <div class="col-lg-8">
          <div class="nf-card">
            <div class="nf-card-head"><h3 class="nf-card-title">Testimony Details</h3></div>
            <div class="nf-card-body">
              <div class="nf-row" style="margin-bottom:16px;">
                <div class="nf-col-half">
                  <label class="nf-label"><?= $locale['name_testifier'] ?></label>
                  <input type="text" name="testifier" class="nf-input" placeholder="<?= $locale['name_testifier'] ?>">
                </div>
                <div class="nf-col-half">
                  <label class="nf-label"><?= $locale['testimony_title'] ?></label>
                  <input type="text" name="title" class="nf-input" placeholder="<?= $locale['testimony_title'] ?>" required>
                </div>
              </div>
              <div>
                <label class="nf-label"><?= $locale['testimony_content'] ?></label>
                <textarea class="editor1" name="content"><?= $locale['add_testimony_content'] ?></textarea>
              </div>
            </div>
          </div>
        ...
        <div class="col-lg-4">
          <div class="nf-card" style="background:linear-gradient(135deg,#fff7ed,#fffbeb);">
            <div class="nf-card-body" style="padding:24px;">
              <div style="display:flex;align-items:flex-start;gap:14px;">
                <div style="width:40px;height:40px;border-radius:12px;background:#f59e0b;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><i class="dw dw-star" style="color:#fff;font-size:1rem;"></i></div>
                <div>
                  <p style="font-size:.85rem;font-weight:700;color:#92400e;margin:0 0 6px;">Note</p>
                  <p style="font-size:.78rem;color:#b45309;margin:0;line-height:1.5;">New testimonies are set to <strong>Pending</strong> by default. An admin must approve before the testimony appears in the app.</p>
                </div>
              </div>
            </div>
          </div>
